...
**8 怪兽名册(2026-07-16 扩编)**:`data.js KAIJUS` 集中登记 8 只怪兽(老两只 `unicorn`=怪兽1独角兽型 / `birdon`=怪兽2鸟型,+ codex `monster-sources` 新六只 `baltan/gomora/kingjoe/redking/fiveking/orochi`)。每只带 `file`(战斗立绘)/`native`/`art`(立绘键)/`icon`(monster-moves 图标组前缀)/`tab`(图鉴头像)/`name/cn/quote`。`RIVAL_OF` 逐英雄指派 CPU 对手(按原作宿敌:初代-巴尔坦/赛文-金古乔/赛罗-哥莫拉/泰罗-雷德王/迪迦-五帝王/泽塔-玛迦大蛇;戴拿/盖亚沿用老两只)。`STILLS[cid].rival` = `Object.assign({}, KAIJUS[RIVAL_OF[id]])`。**取景键从按英雄 cid 改为按怪兽 art**:`stillCrop['k:'+art]`、`bustArt` 里 `ua['kaiju'+art]`、HUD 里 `rvArt = STILLS[cid].rival.art`。烘焙脚本 `tools/bake_kaiju_stills.js`(白底 flood-fill 抠图 + 最大连通域,身高 265 对齐老两只,脚底 y=303.5,过宽按宽 312 封顶;HUD k: 值顶部锚定公式,已对老两只验算 w=93.1 吻合)。

**光之对决·本地双人(2026-07-16, agent/ultraman-local-duel 合入)**:`js/ultraman.js` 独立模块 `UltramanMode`(标题第 3 项 `G.screen='ultraman'`)。选角池 = 全部 8 英雄 + 8 怪兽(4×4),复用主战引擎烘焙立绘(选人 = ui-lab portrait webp,战斗 = 320 方格 still,脚底比 303.5/320,按 `nat` 翻转)。旧 `assets/img/ultraman/*.png` 白底大图**已删**。60s 计时/血条/能量光线/胜负结算,与主战引擎解耦(自成一套简化物理)。`input.js` 已加小键盘键位。
